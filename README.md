...
> **提示**：训练过程会自动保存模型权重至当前目录下的 `Gobang_AlphaZero_V2_model.pth`。如果你将已有的训练模型放置在同级目录下，程序在运行时会自动识别并加载以供继续训练或人机实战。**当前版本为训练约14000轮的模型，后续会发布新版本**
